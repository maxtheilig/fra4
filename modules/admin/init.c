...
void init_gauge(int flag)
{
   ...
   for(ind=0,zu=iu;ind<VOL;ind++)
   {
	 ...
	 {
#if(SUN==3)
	    random_su3(pu[ind][dir]);
#elif(SUN==2)
            random_su2(pu[ind][dir]);
#endif
	 }
      }
   }
...
void copy_gauge_field(sun_mat *u1[VOL][DIM],sun_mat *u2[VOL][DIM])
{
   int ii,jj;
   for(ii=0;ii<VOL;ii++)
      for(jj=0;jj<DIM;jj++)
...
void alloc_wferm(sun_wferm **f)
{
	int x;
	sun_wferm *speicher;
	checkpoint("allocate_wferm");

	speicher = malloc(VOL*sizeof(sun_wferm));
	error(speicher==NULL,"alloc_wferm [start.c]",
	         "Unable to allocate wferm array!");
	for(x=0; x<VOL; x++)
	{
		f[x] = speicher + x;
	}
