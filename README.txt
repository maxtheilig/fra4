...
The program has two directories with executable programs:
main and main_test
In both directories there is a Makefile that can be used to generate the
executables (qcd and test, respectively). The Makefiles can be extended
to include additional files and folders.

To execute either of the programs you can go to the particular directories
and type:
./<prog> -i test.in
The file following '-i' is th input file hich is needed to hand over the
necessary parameters to the program. In both directories a sample infile
'test.in' is included.
